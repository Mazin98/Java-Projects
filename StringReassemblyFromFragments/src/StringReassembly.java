...
public final class StringReassembly {
    ...
    public static void addToSetAvoidingSubstrings(Set<String> strSet,
        ...
        for (int i = 0; i < strSet.size() && check; i++) {
        ...
        if (check) {
            ...
            for (int i = 0; i < strSet.size(); i++) {
                ...
                if (!str.contains(strTemp)) {
                    strSet.add(strTemp); //add to input if not a substring
                }
            }
            strSet.add(str); //add string
        } else {
            //add all strings from temp
            strSet.add(setTemp);
        }
    }
    ...
    public static Set<String> linesFromInput(SimpleReader input) {
        ...
        //reads every input
        while (!input.atEOS()) {
            string = input.nextLine(); //declares string
            addToSetAvoidingSubstrings(set, string); //adds to avoid the substrings
        }
    ...
    public static void printWithLineSeparators(String text, SimpleWriter out) {
        ...
        for (int i = 0; i < text.length(); i++) {
            if (text.charAt(i) == tilde) { //if charater is
                out.println();
            } else {
